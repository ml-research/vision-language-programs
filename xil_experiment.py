...
def get_correct_dsl(prompter, variables, args):
    # get dsl
    if args.xil_remove_confounders:
        if args.xil_add_functions:
            print("Removing confounders and adding XIL functions to the DSL.")

            semantics, primitive_types = get_dsl(
                args.model, "xil-add-functions-no-metal-sphere", prompter, variables
            )
        else:
            print("Removing confounders from the DSL.")
            semantics, primitive_types = get_dsl(
                args.model, "xil-no-metal-sphere", prompter, variables
            )
    else:
        if args.xil_add_functions:
            print("Adding XIL functions to the DSL.")
            semantics, primitive_types = get_dsl(
                args.model, "xil-add-functions", prompter, variables
            )
        else:
            print("Using base DSL without modifications.")
            semantics, primitive_types = get_dsl(
...
def clean_variables(class_id, variables):
    # filter variables to remove confounders
    if class_id in [0, 1, 2]:
        # remove gray/grey variables
        print("Removing gray/grey variables from discovered variables.")
        objects = variables.get("objects", [])
        properties = variables.get("properties", [])

        objects = [
            o.replace("gray ", "").replace("grey ", "").replace("silver ", "")
            for o in objects
        ]
        properties = [
            p
            for p in properties
            if "gray" not in p and "grey" not in p and "silver" not in p
        ]

        variables["objects"] = objects
        variables["properties"] = properties
    if class_id in [0, 1, 2]:
        # remove metal sphere variables from objects
        print("Removing metal sphere variables from discovered variables.")
        objects = variables.get("objects", [])
        ...
        objects = [
            ...
            if not (
                "metal sphere" in o
                or "metallic sphere" in o
                or "shiny sphere" in o
                or "metal ball" in o
                or "metallic ball" in o
                or "shiny ball" in o
                or "metal egg" in o
                or "metallic egg" in o
                or "shiny egg" in o
            )
        ]
...
def main(args):
    ...
    for i, sample in tqdm(enumerate(data)):
        ...
        # get dsl
        semantics, primitive_types = get_correct_dsl(prompter, variables, args)

        # secure dsl without variables
        ...
        # merge original variables with new variables
        for var_type in original_variables:
            original_set = set(original_variables[var_type])
            new_set = set(variables.get(var_type, []))
            merged_set = original_set.union(new_set)
            variables[var_type] = list(merged_set)

        # updated variables after pre-collecting
        print("Discovered variables:")
        for var_type, var_list in variables.items():
            print(f"{var_type}: {var_list}")

        if args.xil_remove_confounders:
            variables = clean_variables(i, variables)
            print("Variables after removal:")
            for var_type, var_list in variables.items():
                print(f"{var_type}: {var_list}")

        img_obj_representations[i] = img_object_representations

        # get dsl with updated variables Note: not done in main experiments, so kept commented out
        # problem_semantics, problem_primitive_types = get_correct_dsl(
        #     prompter, variables, args
        # )

        if args.xil_add_properties:
            # add the property "metallic" to properties if not already present
            if "metallic" not in variables.get(
                "properties", []
            ) or "metal" not in variables.get("properties", []):
                print("ADDING ADDITIONAL PROPERTY 'metallic'...")
                if "properties" in variables:
                    variables["properties"].append("metallic")
                else:
                    variables["properties"] = ["metallic"]

        # fresh dsl with cleaned variables
        add_variables_to_dsl(semantics, primitive_types, variables)
        ...
        print("Starting final pre-collection of image representations...")
        pre_collect_img_repr_final(examples, semantics, variables)
        ...
        if args.xil_remove_confounders and args.xil_add_functions:
            ...
            if args.xil_add_properties:
                top_folder += "/add_functions_and_add_properties"
