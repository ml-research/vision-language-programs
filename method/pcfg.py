...
from method.program import Program, Function, Variable, BasicPrimitive, New
...
class PCFG:
    ...
    def get_sbsur_sampler(self, S=None, seed=None):
        ...
        def get_logprobs(sequence):
            ...
            return np.log(
                np.array(
                    [self.rules[current][P][1] for P in self.list_derivations[current]],
                    dtype=float,
                )
            )

        gen = SequenceGenerator(
            lambda x: [get_logprobs(el) for el in x], max_categories, seed
        )
        ...
        def seq2prog(sequence):
            ...
            while context_stack:
                current = context_stack.pop()
                assert (
                    len(self.list_derivations[current]) == 1
                ), f"Current: {current} has more than 1 derivation:{self.list_derivations[current]}"
                P = self.list_derivations[current][0]
                args_P, w = self.rules[current][P]
                program = (P, program)
                for arg in args_P:
