...
class DSL:
    ...
    def __init__(self, semantics, primitive_types, no_repetitions=None):
        ...
        for p in primitive_types:
            if not p:
                print("Primitive type is empty, skipping")
                continue
            formatted_p = str(p)
            if formatted_p in semantics:
                ...
                print(f"Primitive {formatted_p} not found in semantics")
    ...
    def DSL_to_CFG(
        ...
        while len(list_to_be_treated) > 0:
            ...
            if depth == max_program_depth - 1:
                ...
                for P in self.list_primitives:
                    if (
                        isinstance(P, BasicPrimitive)
                        and P.primitive in self.no_repetitions
                        and context
                        and len(context) > 0
