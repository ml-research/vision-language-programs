from method.cons_list import cons_list2list
import typing
import ray
...
import logging
from method.program import BasicPrimitive, Function, New, Program, Variable
import time
from typing import Callable, Tuple
import method.grammar_splitter
from method.Algorithms.ray_parallel import start, make_parallel_pipelines

from method.Algorithms.heap_search import heap_search
from method.Algorithms.heap_search_naive import heap_search_naive
from method.Algorithms.a_star import a_star
from method.Algorithms.threshold_search import threshold_search
from method.Algorithms.dfs import dfs
from method.Algorithms.bfs import bfs
from method.Algorithms.sort_and_add import sort_and_add
from method.Algorithms.sqrt_sampling import sqrt_sampling, sqrt_sampling_with_sbsur

from method.program_as_list import reconstruct_from_compressed
...
reconstruct = {
    ...
    a_star,
    sort_and_add,
    sqrt_sampling_with_sbsur,
}


def run_algorithm(
    is_correct_program: Callable[[Program, bool], bool],
    pcfg: PCFG,
    algo_index: int,
    timeout: int,
    n_candidates: int = 10,
) -> Tuple[Program, float, float, int, float, float]:
    """
    ...
    print(f"Starting search with {name_algo} for {timeout} seconds...")
    while (
        search_time + evaluation_time < timeout and nb_programs < total_number_programs
    ):
        # print("search_time=", search_time, "evaluation_time=", evaluation_time)
        ...
        try:
            ...
            search_time += time.perf_counter()
            logging.debug("Output the last program after {}".format(nb_programs))
        ...
        if len(program_candidates) < n_candidates:
            ...
            min_probability = min([candidate[2] for candidate in program_candidates])
            ...
            if accuracy > min_acc:
                ...
                if probability > min_probability:
                    # get the candidates with the least accuracy and the least cumulative probability
                    min_candidate = min(
                        program_candidates,
                        key=lambda x: (x[0], x[2]),
                    )
                    if min_candidate[2] < probability:
                        # remove the least accurate candidate with the least cumulative probability
                        program_candidates.remove(min_candidate)
                        program_candidates.append((accuracy, program_r, probability))

            # if all accuracies are 1, we are done
            if all([candidate[0] == 1 for candidate in program_candidates]):
                found = True
        ...
        if found:
            ...
            for accuracy, program_r, c_p in program_candidates:
                print("\taccuracy=", accuracy, "\tprogram=", program_r, "\tc_p=", c_p)
...
def run_algorithm_parallel(
    is_correct_program: Callable[[Program, bool], bool],
    pcfg: PCFG,
    algo_index: int,
    splits: int,
    n_filters: int = 4,
    transfer_queue_size: int = 500_000,
    ...
    grammar_split_time += time.perf_counter()
    make_generators = [
        bounded_generator(None, pcfg, i) for i, pcfg in enumerate(splits)
    ]
    ...
    logging.debug("\tFinished search found={}. Now shutting down...".format(found))
    search_times, cumulative_probabilities, nb_programs = ray.get(
        data_collector.search_data.remote()
    )
    evaluation_times, evaluated_programs = ray.get(
        data_collector.evaluation_data.remote()
    )
    logging.debug(
        "\tStats: found={} generated programs={} evaluated programs={} covered={:.1f}%".format(
            found,
            sum(nb_programs),
            sum(evaluated_programs),
            100 * sum(cumulative_probabilities),
        )
    )
...
def gather_data(
    dataset: typing.List[Tuple[str, PCFG, Callable]], algo_index: int
) -> typing.List[Tuple[str, Tuple[Program, float, float, int, float, float]]]:
    ...
    for task_name, pcfg, is_correct_program in dataset:
        ...
        if isinstance(task_name, Program):
            try:
                ...
                assert (
                    data[0] is not None or algorithm != heap_search or prob < data[-1]
                )
...
def gather_data_parallel(
    dataset: typing.List[Tuple[str, PCFG, Callable]],
    algo_index: int,
    splits: int,
    n_filters: int = 4,
    transfer_queue_size: int = 500_000,
