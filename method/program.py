...
from method.cons_list import index
...
class Program:
    ...
    def make_all_constant_variations(self, constants_list):
        ...
        return [
            self.derive_with_constants(list(possibility))
...
class Function(Program):
    def __init__(self, function, arguments, type_=UnknownType(), probability={}):
        ...
        self.type = type_
        self.hash = hash(
            tuple([arg.hash for arg in self.arguments] + [self.function.hash])
        )
    ...
    def derive_with_constants(self, constants):
        return Function(
            self.function.derive_with_constants(constants),
            [argument.derive_with_constants(constants) for argument in self.arguments],
            self.type,
            self.probability,
        )

    def get_program_depth(self):
        if len(self.arguments) == 0:
            return 0
        return 1 + max([arg.get_program_depth() for arg in self.arguments])
...
class BasicPrimitive(Program):
    def __init__(
        self, primitive, type_=UnknownType(), probability={}, constant_evaluation=None
    ):
        # assert isinstance(primitive, str)
        self.primitive = primitive
        # assert isinstance(type_, Type)
        self.type = type_
        self.is_a_constant = not isinstance(type_, Arrow) and primitive.startswith(
            "constant"
        )
    ...
    def derive_with_constants(self, constants):
        if self.is_a_constant:
            return BasicPrimitive(
                self.primitive, self.type, self.probability, constants.pop()
            )
        else:
            return self
...
class New(Program):
    ...
    def derive_with_constants(self, constants):
        return New(
            self.body.derive_with_constants(constants), self.type, self.probability
        )
