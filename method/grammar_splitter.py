...
from method.type_system import PrimitiveType, Type
...
def __pcfg_from__(original_pcfg: PCFG, group: List[Node]) -> PCFG:
    ...
    rule_no: int = (
        max(key[1][1] if key[1] else 0 for key in original_pcfg.rules.keys()) + 1
    )
    mapping: Dict[Tuple, Context] = {}
    ...
    rules = {key: rules[key] for key in sorted(list(rules.keys()), key=lambda x: x[2])}
...
def __split_nodes_until_quantity_reached__(pcfg: PCFG, quantity: int) -> List[Node]:
    """
...
def __is_fixing_any_hole__(pcfg: PCFG, node: Node, holes: List[Context]) -> bool:
    current = node.program[:]
    stack = [pcfg.start]
    while current:
        S = stack.pop()
...
def __split_into_nodes__(
    ...
    prob_groups = sorted(
        [[g, p] for g, p in zip(groups, masses)], key=lambda x: x[1]
    )  # type: ignore
...
def split(pcfg: PCFG, splits: int, alpha: float = 1.1) -> Tuple[List[PCFG], float]:
    """
