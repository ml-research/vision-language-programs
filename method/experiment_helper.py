...
import method.type_system
from method.Predictions.models import RulesPredictor, BigramsPredictor
from method.pcfg import PCFG
from typing import Callable, List, Tuple
from method.dsl import DSL
from method.program import Program
    ...
    def checker(prog: Program, use_cached_evaluator: bool) -> bool:
        ...
        if use_cached_evaluator:
            for i, example in enumerate(examples):
                ...
                if output != out and allowed_incorrect <= 0:
                    return False
                elif output != out:
                    allowed_incorrect -= 1
            return True
...
def make_program_checker_with_constants(
    dsl: DSL, examples, constants
) -> Callable[[Program, bool], Tuple[bool, Program]]:
...
def task_set2dataset(
    tasks, model, dsl: DSL
) -> List[Tuple[str, PCFG, Callable[[Program, bool], bool]]]:
    dataset = []
    batch_IOs = []
    batch_types = []
    # Prepare batch
    ...
    try:
        ...
        print(
            "experiment_helper.py: task_set2dataset: An error occured while generating grammars:\n\t",
            e,
        )
        return []
    # Reconstruction
    ...
    if isinstance(model, BigramsPredictor):
        grammars = model.reconstruct_grammars(grammars, batch_types, tensors=False)
        ...
        dataset.append(
            (
                ...
                (
                    ...
                    if constants
                    else make_program_checker(dsl, examples)
                ),
            )
        )
    return dataset
...
def task_set2uniform_dataset(
    tasks, dsl: DSL, max_program_depth: int = 4
) -> List[Tuple[str, PCFG, Callable[[Program, bool], bool]]]:
    dataset = []
    # Prepare batch
    for task in tasks:
...
def filter_examples(examples, nb_arguments_max, max_list_size, lexicon, verbose=False):
    ...
    for i, o in examples:
        ...
        if hasattr(o, "__len__") and any(x not in lexicon for x in o):
            if verbose:
                print(
                    "\toutput iterable not in lexicon:",
