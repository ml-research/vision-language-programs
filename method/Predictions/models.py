...
from method.program import Function, Variable, BasicPrimitive, New
...
class RulesPredictor(nn.Module):
        ...
        for x in batch_predictions:
            ...
            for S in self.cfg.rules:
                ...
                for P in self.cfg.rules[S]:
                    ...
                    rules[S][cpy_P] = self.cfg.rules[S][P], float(
                        x[self.RuleToIndex[(S, P)]]
                    )
            grammar = PCFG(
    ...
    def custom_collate(self, batch):
        return [batch[i][0] for i in range(len(batch))], torch.stack(
...
class NNDictRulesPredictor(nn.Module):
    ...
    def __init__(self, cfg, IOEncoder, IOEmbedder, latent_encoder):
        ...
        self.loss = lambda batch_grammar, batch_program: -sum(
            grammar.log_probability_program(grammar.start, program)
        ...
        for S in self.cfg.rules:
            ...
            module = nn.Sequential(
                nn.Linear(output_size, n_productions), nn.LogSoftmax(-1)
            )
    ...
    def forward(self, batch_IOs):
        ...
        for x in batch_IOs:
            ...
            x = self.latent_encoder(x)
            probabilities = {
                S: self.projection_layer[format(S)](x) for S in self.cfg.rules
            }
            rules = {}
            for S in self.cfg.rules:
                rules[S] = {}
                for j, P in enumerate(self.cfg.rules[S]):
                    cpy_P = copy.deepcopy(P)
                    rules[S][cpy_P] = self.cfg.rules[S][P], probabilities[S][0, j]
            grammar = LogProbPCFG(
                self.cfg.start, rules, max_program_depth=self.cfg.max_program_depth
            )
            grammar.clean()
...
class BigramsPredictor(nn.Module):
    ...
    def __init__(
        ...
        self.maximum_arguments = max(
            len(t.arguments()) for t in self.primitive_types.values()
        )
        self.q_predictor = nn.Linear(
            output_size,
    ...
    def forward(self, batch_IOs):
        ...
        x = self.IOEmbedder.forward(batch_IOs)
        x = self.latent_encoder(x)
        x = self.q_predictor.forward(x).view(
            -1,
    ...
    def reconstruct_grammars(
        self, batch_predictions, batch_type_requests, tensors=True
    ):
        grammars = []
        for x, type_request in zip(batch_predictions, batch_type_requests):
            ...
            for S in cfg.rules:
                    ...
                    if isinstance(P, (BasicPrimitive, New)):
                        ...
                        if tensors:
                            ...
                            rules[S][cpy_P] = (
                                cfg.rules[S][P],
                                x[
                                    parent_index, argument_number, primitive_index
                                ].item(),
                            )
                ...
                if variables:
                    ...
                    if total > 0:
                        # Normalise rest
                        to_add = np.log(1 - self.variable_probability) - np.log(total)
                        for P in rules[S]:
            ...
            grammar = LogProbPCFG(
                cfg.start, rules, max_program_depth=cfg.max_program_depth
            )
            grammar.clean()
            grammars.append(grammar)
        return grammars
