...
class Type:
    ...
    def __eq__(self, other):
        ...
        b2 = b2 or (
            isinstance(self, PolymorphicType)
            and isinstance(other, PolymorphicType)
        ...
        b2 = b2 or (
            isinstance(self, Arrow)
            and isinstance(other, Arrow)
            and self.type_in.__eq__(other.type_in)
            and self.type_out.__eq__(other.type_out)
        )
        b2 = b2 or (
            isinstance(self, List)
            and isinstance(other, List)
            and self.type_elt.__eq__(other.type_elt)
        )
        return b and b2
    ...
    def list_ground_types(self, polymorphic=False):
        if isinstance(self, Arrow):
            return self.type_in.list_ground_types(
                polymorphic
            ) + self.type_out.list_ground_types(polymorphic)
    ...
    def size(self):
        ...
        if isinstance(self, List) and isinstance(
            self.type_elt, (PrimitiveType, PolymorphicType)
        ):
            return 2
        if (
            isinstance(self, List)
            and isinstance(self.type_elt, List)
    ...
    def unify_rec(self, other, dic):
        ...
        if isinstance(self, Arrow):
            return (
                ...
                and self.type_in.unify_rec(other.type_in, dic)
                and self.type_out.unify_rec(other.type_out, dic)
            )
        if isinstance(self, List):
            return isinstance(other, List) and self.type_elt.unify_rec(
                other.type_elt, dic
            )
...
IMG = PrimitiveType("img")
OBJECT = PrimitiveType("object")
ACTION = PrimitiveType("action")
PROPERTY = PrimitiveType("property")
SCENERY = PrimitiveType("scenery")
CONDITION = PrimitiveType("condition")
MODEL = PrimitiveType("model")
